...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      // Check if this is a request for latest products
      const isLatestRequest = req.url.includes('/latest');
      const limit = isLatestRequest ? (parseInt(req.query.limit) || 4) : null;
      
      console.log(isLatestRequest 
